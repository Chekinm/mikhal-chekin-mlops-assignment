...
|---|---|---|
| `--max-model-len` | 8192 | Prompts are 1.5–3K tokens (schema + question) with short SQL output; 8K leaves headroom without wasting KV. I set this parameter to 16384 initially to fit long answers, but finally after some fixes in agent logic and adding some restrictions for big schemas I realized that we don't need such a big context, so I reduce it back to 8192.|
| `--max-num-seqs` | 64 | Each agent run fans out to ~2–3 dependent vLLM calls; 64 covers 10 RPS × fan-out. Confirmed sufficient: queue stayed empty under load. Played with this number during experiments. Making it smaller affects the performance, while 32 to 64 make quite a small difference. I suggest staying at 64.|
| `--max-num-batched-tokens` | 4096 | rationale — chunked-prefill batch budget |
...
| `--enable-chunked-prefill` | true | overlaps prefill with decode to smooth latency |
...
**Commentary.** With the implemented agent flow. the revise loop barely earns its keep: iter 1 adds exactly one correct
answer (+3.3pp), iter 2 adds zero. The architecture is doing minimal work on this set.
So as you will see later we set max iteration to 2 as the 3-step run does not achieve any better results, while making the P99 latency significantly slower.
...
iteration is pure latency cost. More improvements can come from improving the agent.. 
We need to figure out how to handle huge schemas. Probably use some small and fast model to trim the schema we feed to the main model to generate SQL.
...
The metrics are a bit mixed as we started with 5 RPS and only the final runs were run at 10 RPS. 
...
```
You can see 2 timeouts and 77 HTTP errors here — not good at all.
Most of the HTTP errors were caused by a bug in this function. It had no guard for None values, so it just failed. Adding default for None value get rid of the HTTP errors.
```python
def _q(ident) -> str:
...
The base accuray of the system is 
```
"summary": {
    "total": 30,
...
```
Not a big number. But as our goal here is o11y, I don't spent much time on agent improvment.

**Iteration #1 - #9.**

After fixing the Python code I still ran into a timeout problem. Some agent calls ran for 3 to 4 minutes. Investigation and debugging lead me to this three solutionsnvestigation and debugging led me to these three fixes. 
1. Trancate huge DB schemas (expesialy the football one)
2. Truncate long lines to 4000 characters.
...
After this I ended up with the numbers below with proud zeros next to timeouts and errors.
```
  "summary": {
    "requested_rps": 5.0,
    "duration_seconds": 120,
...
```
**Iteration #10 - #14.**
After fixeing the flow I started optimizing the preformance.
I checked how changing --max-num-seqs affects the timings.. 
Setting small numbers like 16 queued our calls, which increased the time even for P50 up to 5 s.
Stop at 64. 
Then I played with the length of the verify_node LLM call.. 
Originally all calls to the LLM had a 1024 max_len limit.. 
Setting it to 128 for the verify call (a simple ok / not-ok with some explanation) reduced overall timing significantly. 
```"summary": {
    "requested_rps": 5.0,
    "duration_seconds": 120,
...
**Iteration #15 - #20.**
Here I decided to challenge the system with 10 RPS loads, so the values got slightly worse.
In this step the biggest improvement came from running uvicorn on 4 workers.
And reducing --max-model-len to 8192 (it was set to 16384 while fighting timeouts at the beginning).
These two things lead us to these numbers
...
Here I found missing slash in model configuraiton. Due to this missing / we one of the flag left out of reach for bash.
Bascily we work without chucked_preffill. 
So I fix it and seting it to true reduce the latency by 1.5 seconds. 
...
```
I consider this the final serving config.
...
```
This shows the 3rd loop run does not improve results, while it does affect latency. 

I ran one more load test with max_iteration=2. And get this final number for latency. This is quite close to the 5 s goal, but I reached it by cutting the loop count to 2 — which in my opinion is not entirely fair.
```
...
### Final result
I take the first 10 RPS run as the starting point, so the real improvement is a bit higher.
...
Additional work on the agent logic itself is needed to improve first-pass generation and handle massive schemas, which affect timing significantly.
